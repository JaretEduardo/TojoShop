...
let cfg = {};
try { cfg = JSON.parse(fs.readFileSync(configPath, 'utf8')); } catch(e) { console.log(JSON.stringify({ type:'config_error', message:'No se pudo leer env.json', error:e.message })); }
...
const FEED_NAME = (cfg.AIO_FEEDS && cfg.AIO_FEEDS.presencia) ? cfg.AIO_FEEDS.presencia : 'sensor-presencia';
...
client.on('connect', () => {
    ...
    client.subscribe(topic, (err) => {
        console.log(JSON.stringify({
            type: err ? 'subscribe_error' : 'subscribed',
            feed_name: FEED_NAME,
            topic,
...
console.log(JSON.stringify({
    ...
    username: AIO_USERNAME,
    url: ADAFRUIT_IO_URL,
    has_key: AIO_KEY !== 'MISSING_AIO_KEY',
    timestamp: new Date().toISOString()
}));